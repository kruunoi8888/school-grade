...
const GradeRow = React.memo(({ 
  ...
  handleKeyDown, handlePaste, calcGPA, deleteStudentGrades, getKStyles, calculateGradeResult, gradeKey, setIsEditMode
}) => {
                  ...
                  <div style={{position:"relative"}}>
                    ...
                    {val && !isNaN(parseFloat(val)) && (
                      <div style={{
                        ...
                        fontWeight:900, color:"#4f46e5", border:"1px solid #e2e8f0", zIndex:5
                      }}>
...
export default function GradeManagePage({ 
  ...
  useEffect(() => {
    ...
    else { setStudents([]); setSubjects([]); setGrades({}); setDraftGrades({}); }
  ...
  async function fetchClassroomData() {
    try {
      ...
      grdData.forEach(g => {
        ...
        const subIdForMap = g.activity_id ? `act_${g.activity_id}` : g.subject_id;
        const key = gradeKey(g.student_id, subIdForMap, g.semester);
        const val = String(g.score || g.grade || "");
        grdMap[key] = val;
        initialDraft[`${g.student_id}_${type}_${tid}_${g.semester}`] = val;
      });
      setGrades(grdMap);
  ...
  const handleDeleteAllClassroomGrades = async () => {
    if (!filterClass) return;
    const selectedClass = classrooms.find(c => c.room_name === filterClass);
    if (!selectedClass) return;
    if (!window.confirm("🔴 คำเตือน: คุณแน่ใจที่จะลบเกรดนักเรียนทั้งห้องใช่หรือไม่?")) return;
    if (!window.confirm("⚠️ ยืนยันขั้นสุดท้าย: ลบเกรดทั้งหมดในห้อง " + filterClass + " ใช่ไหม?")) return;
    try {
      setSaving(true);
      const studIds = students.map(s => s.id);
      if (studIds.length === 0) return;
      let query = supabase.from('grades').delete().in('student_id', studIds).eq('academic_year_id', activeYear.id);
      if (isSemesterMode) query = query.eq('semester', currentSem);
  ...
  return (
    <div className="adm-container" style={{maxWidth:1140}}>
            ...
            <table style={{width:"100%", borderCollapse:"collapse"}}>
              <thead>
                <tr style={{background:"#f8fafc"}}>
                  ...
                  {!activeCurriculum.includes('kindergarten') && <th style={{padding:12, borderBottom:"2px solid #e2e8f0"}}>GPA</th>}
                  <th style={{padding:12, textAlign:"center", borderBottom:"2px solid #e2e8f0"}}>
                    <div style={{fontSize:10, color:"#94a3b8", marginBottom:4}}>จัดการ</div>
                    <button 
                      onClick={handleDeleteAllClassroomGrades} 
                      title="ลบเกรดทั้งหมดในห้องนี้"
                      style={{
                        padding:6, borderRadius:8, border:"1px solid #fee2e2", 
      ...
      {success && <div style={{position:"fixed", bottom:20, right:20, background:"#10b981", color:"#fff", padding:"10px 20px", borderRadius:10, zIndex:1000}}>Saved!</div>}
